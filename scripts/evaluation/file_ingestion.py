...
class FileIngestionEngine:
    ...
    def _get_file_metadata(self, file_path: Path, source: str = 'local') -> FileMetadata:
    ...
    def ingest_local_directory(self, directory: Path, recursive: bool = True) -> Dict[str, Any]:
    ...
    def ingest_dropbox_folder(self, dropbox_client: dropbox.Dropbox, 
                            folder_path: str,
                ...
                for file_metadata in file_list:
                    try:
                        ...
                        with open(local_path, 'wb') as f:
                            _, response = dropbox_client.files_download(file_metadata.path_display)
...
def test_ingestion_system():
    ...
    local_results = engine.ingest_local_directory(
        Path("/workspaces/amy-project/hansman_organized/downloads"),
        recursive=False
    )
