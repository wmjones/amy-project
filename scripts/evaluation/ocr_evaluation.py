...
class OCRBenchmark:
    ...
    def prepare_test_samples(self, sample_folder="/workspaces/amy-project/hansman_organized/downloads"):
        ...
        try:
            ...
            for file_path in self.sample_files:
                try:
                    ...
                    print(f"  ✓ {file_path.name}: {len(text)} chars in {processing_time:.2f}s")
            ...
            if tesseract_results['processing_times']:
                tesseract_results['summary'] = {
                    'avg_processing_time': np.mean(tesseract_results['processing_times']),
                    'avg_confidence': np.mean(tesseract_results['confidence_scores']),
                    'success_rate': len(tesseract_results['processing_times']) / len(self.sample_files),
                    'cost': 0.0  # Free
                }
...
"""
        ...
        for solution, data in sorted(report['decision_matrix'].items(), 
                                   key=lambda x: x[1]['weighted_score'], 
                                   reverse=True):
