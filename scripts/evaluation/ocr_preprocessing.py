...
class DocumentPreprocessor:
    ...
    def create_preprocessing_pipeline(self, metrics: ImageQualityMetrics) -> List[str]:
        ...
        if (
            ...
            or metrics.resolution[1] < self.quality_thresholds["min_resolution"][1]
        ):
        ...
        for step in pipeline:
            if step in self.techniques:
                ...
                if save_intermediate:
                    step_path = (
                        image_path.parent
                        / f"{image_path.stem}_{step}{image_path.suffix}"
                    )
    ...
    def batch_process(
        self, input_folder: Path, output_folder: Path, sample_size: Optional[int] = None
    ) -> Dict:
        ...
        # Get image files
        image_files = list(input_folder.glob("*.JPG")) + list(
        ...
        for image_path in image_files:
            try:
                ...
                output_path = (
                    output_folder / f"{image_path.stem}_processed{image_path.suffix}"
                )
                ...
                statistics["failed"].append({"file": image_path.name, "error": str(e)})
    ...
    def optimize_for_ocr_engine(
        self, image_path: Path, ocr_engine: str = "tesseract"
    ) -> np.ndarray:
        ...
        if config["prefer_binary"] and len(final_image.shape) == 3:
            ...
            _, final_image = cv2.threshold(
                final_image, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
            )
...
def create_comparison_report(
    preprocessor: DocumentPreprocessor, test_folder: Path, output_path: Path
) -> None:
    ...
    fig, axes = plt.subplots(
        len(sample_images), 4, figsize=(16, 4 * len(sample_images))
    )
    ...
    for idx, image_path in enumerate(sample_images):
        ...
        if "enhance_contrast" in results["steps"]:
            axes[idx, 1].imshow(
                cv2.cvtColor(results["steps"]["enhance_contrast"], cv2.COLOR_BGR2RGB)
            )
        ...
        if "deskew" in results["steps"]:
            axes[idx, 2].imshow(
                cv2.cvtColor(results["steps"]["deskew"], cv2.COLOR_BGR2RGB)
            )
