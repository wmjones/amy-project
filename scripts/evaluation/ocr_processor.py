    ...
    def set(self, file_hash: str, result: Dict):
        ...
        try:
            self.redis_client.setex(
                f"ocr:{file_hash}", self.expire_time, json.dumps(result, default=str)
            )
        except Exception as e:
...
class OCRProcessor:
    ...
    def process_file(
        self, file_path: Path, file_metadata: Optional[Dict] = None
    ) -> OCRResult:
        """Process a single file through the OCR pipeline."""
        start_time = time.time()
        file_hash = self._calculate_file_hash(file_path)
        file_id = (
            file_metadata.get("file_id", file_hash) if file_metadata else file_hash
        )
        ...
        try:
            ...
            for image, page_num in images:
                ...
                try:
                    ...
                    if confidence >= self.confidence_threshold:
                        ...
                        logger.info(
                            f"Tesseract success for {file_path.name} page {page_num} (confidence: {confidence:.2f})"
                        )
                    else:
                        logger.warning(
                            f"Tesseract confidence too low ({confidence:.2f}) for {file_path.name} page {page_num}"
                        )

                except Exception as e:
                    logger.error(
                        f"Tesseract failed for {file_path.name} page {page_num}: {e}"
                    )
                ...
                if page_confidence < self.confidence_threshold and self.google_client:
                    try:
                        ...
                        temp_path = (
                            file_path.parent
                            / f"temp_{file_path.stem}_page{page_num}.png"
                        )
                        ...
                        if confidence > page_confidence:
                            ...
                            logger.info(
                                f"Google Vision success for {file_path.name} page {page_num} (confidence: {confidence:.2f})"
                            )
                        ...
                        logger.error(
                            f"Google Vision failed for {file_path.name} page {page_num}: {e}"
                        )
            ...
            final_confidence = (
                sum(all_confidence) / len(all_confidence) if all_confidence else 0.0
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tesseract_usage": (
                self.stats["tesseract_success"] / total if total > 0 else 0
            ),
...
def test_ocr_processor():
    ...
    processor = OCRProcessor(
        tesseract_config={"lang": "eng", "config": "--oem 3 --psm 6"},
