...
def compare_results():
    ...
    print(
        f"   Additional tokens: {result_with_image.tokens_used - result_no_image.tokens_used}"
    )
    ...
    summary_no_image = summarizer_no_image.summarize_document(
        file_path=test_image, ocr_text=test_ocr_text
    )
    ...
    print(
        f"     Key Entities: {len(summary_no_image.key_entities.get('people', [])) + len(summary_no_image.key_entities.get('locations', []))} total"
    )
    ...
    print(
        f"     Key Entities: {len(summary_with_image.key_entities.get('people', [])) + len(summary_with_image.key_entities.get('locations', []))} total"
    )

    print("\n" + "=" * 60)
    print("CONCLUSION: Including images provides more accurate document analysis,")
    print(
        "especially when OCR quality is poor or the document contains visual elements."
    )
