...
def test_hansman_pipeline():
    ...
    for image_path in existing_images[:1]:  # Test with first image
        ...
        # Look for OCR text
        ocr_text_path = (
            image_path.parent.parent / "ocr_results" / f"{image_path.stem}_ocr.txt"
        )
        ...
        try:
            ...
            print(
                f"  Location References: {', '.join(summary.location_references[:3])}"
            )
            print(
                f"  Key Entities: {len(summary.key_entities.get('people', [])) + len(summary.key_entities.get('locations', [])) + len(summary.key_entities.get('dates', []))} total"
            )
