...
def load_summaries_to_csv(
    ...
    for json_file in json_files:
        try:
            ...
            if isinstance(quality_indicators, dict):
                summary_data['text_clarity'] = quality_indicators.get('text_clarity', '')
                summary_data['historical_value'] = quality_indicators.get('historical_value', '')
                summary_data['preservation_priority'] = quality_indicators.get('preservation_priority', '')
            else:
            ...
            if isinstance(claude_metadata, dict):
                ...
                summary_data['claude_tokens_used'] = claude_metadata.get('tokens_used', '')
            else:
    ...
    list_columns = [
        ...
        'classification_tags', 'location_references', 'photo_subjects', 
        'related_themes', 'related_documents', 'date_references'
    ]
