...
class HansmanDocumentGrouper:
    ...
    def __init__(self, summaries_dir: str = "/workspaces/amy-project/hansman_full_processing/summaries"):
    ...
    def load_summaries(self) -> pd.DataFrame:
        ...
        for json_file in json_files:
            try:
                ...
                summary_data['historical_value'] = quality_indicators.get('historical_value', 0)
                summary_data['preservation_priority'] = quality_indicators.get('preservation_priority', 0)
    ...
    def group_by_quality(self, quality_threshold: float = 0.7) -> Dict[str, pd.DataFrame]:
        ...
        medium_quality = self.df[(self.df['avg_quality'] >= 0.4) & (self.df['avg_quality'] < quality_threshold)]
    ...
    def find_related_documents(self, file_name: str, similarity_threshold: float = 0.5) -> pd.DataFrame:
        ...
        for idx, row in self.df.iterrows():
            ...
            if source_doc['organizations'] and row['organizations']:
                common_orgs = set(source_doc['organizations']) & set(row['organizations'])
    ...
    def create_visualization(self, output_dir: str = "/workspaces/amy-project/workspace/reports"):
    ...
    def export_groups(self, output_dir: str = "/workspaces/amy-project/workspace/reports"):
