...
class HansmanUnifiedProcessor:
    ...
    def scan_dropbox_folder(self, folder_path: str = "/Hansman Syracuse photo docs July 2015") -> List[Dict[str, Any]]:
    ...
    def organize_file(self, filename: str, metadata: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            doc_metadata = DocumentMetadata(
                ...
                confidence_scores={
                    'overall': metadata.get('confidence_score', 0.0)
                }
            )
    ...
    def run_full_pipeline(self) -> bool:
        ...
        print(f"Using {len(files)} {'local' if self.use_local_files else 'Dropbox'} files")
        ...
        print(f"{'Copied' if self.use_local_files else 'Downloaded'}: {len(download_results)} files")
            ...
            if not self.file_needs_processing(filename, 'ocr'):
                ...
                if ocr_path and Path(ocr_path).exists():
                    try:
                        ...
                        logger.warning(f"Failed to load OCR JSON for {filename}: {e}. Will reprocess.")
                        # Force reprocess this file
                        result = self.process_ocr(filename)
                        if result:
                            ocr_results[filename] = result
                    except Exception as e:
                        logger.warning(f"Error loading OCR for {filename}: {e}. Will reprocess.")
                        result = self.process_ocr(filename)
                        if result:
                            ocr_results[filename] = result
                else:
                    logger.warning(f"OCR path not found for {filename}. Will reprocess.")
                    result = self.process_ocr(filename)
                    if result:
                        ocr_results[filename] = result
            else:
                ...
                if not self.file_needs_processing(filename, 'ai'):
                    ...
                    if ai_path and Path(ai_path).exists():
                        try:
                            ...
                            logger.warning(f"Failed to load AI JSON for {filename}: {e}. Will reprocess.")
                            # Get OCR text
                            ocr_text = ocr_data.get('text', '') if isinstance(ocr_data, dict) else str(ocr_data)
                            result = self.process_ai_summary(filename, ocr_text)
                            if result:
                                ai_results[filename] = result
                                ai_processed += 1
                        except Exception as e:
                            logger.warning(f"Error loading AI summary for {filename}: {e}. Will reprocess.")
                            ocr_text = ocr_data.get('text', '') if isinstance(ocr_data, dict) else str(ocr_data)
                            result = self.process_ai_summary(filename, ocr_text)
                            if result:
                                ai_results[filename] = result
    ...
    def generate_reports(self):
        ...
        try:
            ...
            with open(summary_file, 'w') as f:
                f.write("Hansman Syracuse Collection Processing Summary (OCR + AI Only)\n")
                f.write("=" * 60 + "\n\n")
                ...
                if self.errors:
                    ...
                    for error in self.errors:
                        f.write(f"- {error['stage']}: {error['file']} - {error['error']}\n")
...
def main():
    ...
    # Directory options
    parser.add_argument('--output-dir', default='./hansman_results',
    ...
    parser.add_argument('--force-download', action='store_true',
                       help='Force redownload all files')
    parser.add_argument('--force-ocr', action='store_true',
                       help='Force rerun OCR on all files')
    parser.add_argument('--force-ai', action='store_true',
                       help='Force rerun AI analysis on all files')
