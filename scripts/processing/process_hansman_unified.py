...
class HansmanUnifiedProcessor:
    ...
    def download_files(self, files: List[dropbox.files.FileMetadata]) -> Dict[str, Any]:
        ...
        download_stats = {"downloaded": 0, "skipped": 0, "errors": 0, "files": []}
        ...
        if files_to_download:
            for file_entry in tqdm(files_to_download, desc="Downloading files"):
                try:
                    ...
                    self.errors.append(
                        {"stage": "download", "file": file_entry.name, "error": str(e)}
                    )
    ...
    def initialize_processors(self) -> bool:
        ...
        try:
            ...
            self.organization_engine = OrganizationEngine(
                rules_path=None, use_default_rules=True, default_folder="Unsorted"
            )
    ...
    def generate_final_report(self):
        ...
        summary_file = (
            self.reports_dir / f"summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
        with open(summary_file, "w") as f:
            ...
            f.write(
                f"- Total files: {report_data['processing_summary']['total_files']}\n"
            )
            f.write(
                f"- OCR processed: {report_data['processing_summary']['ocr_processed']}\n"
            )
            f.write(
                f"- AI processed: {report_data['processing_summary']['ai_processed']}\n"
            )
            ...
            if self.errors:
                ...
                for error in self.errors[:10]:  # Show first 10 errors
                    f.write(
                        f"- {error.get('stage')}: {error.get('file', 'N/A')} - {error.get('error')}\n"
                    )
    ...
    def run_full_pipeline(self):
        ...
        if self.use_local_files:
            ...
            print(
                f"Downloaded: {download_stats['downloaded']}, Skipped: {download_stats['skipped']}, Errors: {download_stats['errors']}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process Hansman Syracuse photo collection"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--force-ocr", action="store_true", help="Force re-run OCR on all files"
    )
    parser.add_argument(
        "--force-ai",
        action="store_true",
    ...
    parser.add_argument(
        "--force-organize", action="store_true", help="Force re-organize all files"
    )
    parser.add_argument(
        "--force-all", action="store_true", help="Force re-run all processing steps"
    )
    parser.add_argument(
    ...
    if args.use_local:
        ...
        if not local_path.exists() or not local_path.is_dir():
            parser.error(
                f"Local directory '{args.local_dir}' does not exist or is not a directory"
            )
