    ...
    try:
        ...
        for i, (filename, doc_type, date) in enumerate(source_files):
            ...
            if doc_type == "invoice":
                ...
                dest = temp_dir / "organized" / f"Legal/Contracts/ClientCorp/2023/{filename}"
                rule = rules[3]
            else:
        ...
        print(visualizer.visualize_directory_tree(
        ...
        for path, info in sorted(size_map.items())[:5]:  # Show top 5
            print(f"{path:30s} {info['size_formatted']:>10s} ({info['file_count']} files)")
