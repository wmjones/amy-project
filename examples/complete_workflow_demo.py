...
def demo_complete_workflow():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        config = {
            ...
            "source": {"type": "local", "directory": str(source_dir)},
            "processing": {"batch_size": 5, "file_types": [".txt", ".pdf", ".docx"]},
            "organization": {
                "mode": "copy",
                ...
                result.content = json.dumps(
                    {
                        ...
                        "entities": {"people": ["John Doe", "Dr. Sarah Wilson"]},
        ...
        manipulator = FileManipulator(base_directory=target_dir, dry_run=False)
        ...
        for filename in created_files:
            ...
            try:
                ...
                metadata = extractor.extract_metadata(
                    file_content=content, file_path=str(file_path), file_type="txt"
                )
                ...
                # Copy file
                print("  - Copying file...")
                success = manipulator.organize_file(str(file_path), target_path, "copy")

                if success:
                    print("  ✓ Successfully organized")
                    processed_count += 1
                    progress_tracker.update_progress(
                        filename, "success", {"target": target_path}
                    )
                else:
                ...
                print(f"  ✗ Error: {str(e)}")
                error_count += 1
                progress_tracker.update_progress(filename, "error", str(e))
