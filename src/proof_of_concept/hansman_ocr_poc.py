...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/hansman_poc.log'),
...
class HansmanOCRProofOfConcept:
    ...
    def __init__(self, use_claude: bool = True):
        ...
        if self.use_claude and os.getenv('ANTHROPIC_API_KEY'):
            ...
            if self.use_claude:
                logger.warning("Claude API key not found. Summarization will be skipped.")
    ...
    def get_sample_files(self, sample_dir: str = 'data/hansman_samples', 
    ...
    def extract_text(self, file_path: Path, preprocessed: bool = True) -> Dict[str, Any]:
    ...
    def update_metrics(self, result: Dict[str, Any]):
        ...
        self.metrics['processing_times'][file_name] = {
            ...
            'summarization': result['stages'].get('summarization', {}).get('processing_time', 0),
    ...
    def save_result(self, result: Dict[str, Any]):
        ...
        # Save summary
        if result['stages'].get('summarization', {}).get('success'):
            summary_file = Path('output/summaries') / f"{result['file_name']}_summary.json"
            summary_file.write_text(json.dumps(result['stages']['summarization'], indent=2))
    ...
    def generate_report(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        report = {
            ...
            'summary': {
                ...
                'ocr_success_rate': self.metrics['successful_ocr'] / max(1, self.metrics['total_files']),
                'successful_summaries': self.metrics['successful_summaries'],
                'failed_summaries': self.metrics['failed_summaries'],
                'summary_success_rate': self.metrics['successful_summaries'] / max(1, self.metrics['total_files'])
            },
    ...
    def identify_challenges(self, results: List[Dict[str, Any]]) -> List[str]:
        ...
        if preprocessing_failures > 0:
            challenges.append(f"Image preprocessing failed for {preprocessing_failures} files")
        
        if low_confidence_count > 0:
            challenges.append(f"Low OCR confidence (<70%) for {low_confidence_count} files")
        
        if empty_text_count > 0:
            challenges.append(f"No text extracted from {empty_text_count} files")
        
        if self.metrics['failed_summaries'] > 0:
            challenges.append(f"Summarization failed for {self.metrics['failed_summaries']} files")
        
        return challenges
    
    def get_successful_examples(self, results: List[Dict[str, Any]], limit: int = 3) -> List[Dict[str, Any]]:
        """Get examples of successful processing."""
        successful = []
        
        for result in results:
            if (result['stages']['ocr']['success'] and 
                result['stages'].get('summarization', {}).get('success')):
                example = {
                    'file_name': result['file_name'],
    ...
    def get_failed_examples(self, results: List[Dict[str, Any]], limit: int = 3) -> List[Dict[str, Any]]:
        """Get examples of failed processing."""
        failed = []
        
        for result in results:
            if not result['stages']['ocr']['success'] or \
               not result['stages'].get('summarization', {}).get('success'):
                example = {
                    'file_name': result['file_name'],
                    'ocr_error': result['stages']['ocr'].get('error'),
                    'summary_error': result['stages'].get('summarization', {}).get('error'),
                    'preprocessing_success': result['stages']['preprocessing']['success']
                }
    ...
    def generate_recommendations(self, results: List[Dict[str, Any]]) -> List[str]:
        ...
        if ocr_success_rate < 0.8:
            recommendations.append("Consider additional image preprocessing techniques for better OCR accuracy")
        
        if self.metrics['failed_summaries'] > 0 and not self.summarizer:
            recommendations.append("Configure Claude API credentials for text summarization capabilities")
        
        # Based on performance
        perf_metrics = self.calculate_performance_metrics()
        if perf_metrics.get('average_ocr_time', 0) > 5:
            recommendations.append("Consider using GPU-accelerated OCR for faster processing")
        
        if perf_metrics.get('estimated_time_for_400_files', 0) > 120:  # More than 2 hours
            recommendations.append("Implement parallel processing to reduce total processing time")
        
        # Based on challenges
        challenges = self.identify_challenges(results)
        if any('Low OCR confidence' in c for c in challenges):
            recommendations.append("Use Google Cloud Vision API for documents with low Tesseract confidence")
    ...
    def calculate_cost_estimates(self) -> Dict[str, Any]:
        ...
        claude_input_cost = (num_files * avg_tokens_per_doc * 3) / 1_000_000  # $3 per million
...
def main():
    ...
    print(f"Average Processing Time: {report['performance_metrics']['average_total_time']:.2f} seconds/file")
