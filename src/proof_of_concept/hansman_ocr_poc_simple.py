    ...
    def process_all_samples(self) -> Dict[str, Any]:
        ...
        for sample_file in samples:
            ...
            if result["success"]:
                ...
                logger.info(
                    f"✓ OCR completed for {sample_file.name} "
            ...
            self.metrics["processing_times"][sample_file.name] = result[
                "processing_time"
            ]
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "success_rate": (
                    (self.metrics["successful_ocr"] / self.metrics["total_files"] * 100)
            ...
            "performance": {
                "average_processing_time": (
                    np.mean(list(self.metrics["processing_times"].values()))
                ...
                "processing_speed": (
                    ...
                    / sum(self.metrics["processing_times"].values())
                    if sum(self.metrics["processing_times"].values()) > 0
                    else 0
                ),
            },
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if self.metrics["successful_ocr"] == self.metrics["total_files"]:
            recommendations.append(
                "OCR processing is highly reliable with current settings"
            )
        elif self.metrics["successful_ocr"] / self.metrics["total_files"] > 0.8:
            recommendations.append(
                "OCR processing shows good reliability but may need fine-tuning for some documents"
            )
        else:
            recommendations.append(
                "OCR processing needs significant optimization for this collection"
            )

        avg_confidence = np.mean(
            [
                r["confidence"]
                for r in self.metrics["ocr_results"].values()
                if "confidence" in r
            ]
        )
        if avg_confidence > 90:
            ...
            recommendations.append(
                "Text extraction quality is good but could benefit from image preprocessing improvements"
            )
        else:
            recommendations.append(
                "Consider enhanced image preprocessing to improve OCR accuracy"
            )
...
def main():
    ...
    print(
        f"Average Processing Time: {report['performance']['average_processing_time']:.2f}s"
    )
