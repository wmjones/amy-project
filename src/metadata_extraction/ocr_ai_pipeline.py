...
class OCRAIPipeline:
    ...
    def _reconstruct_from_cache(self, cached_data: Dict[str, Any]) -> PipelineResult:
        ...
        return PipelineResult(
            ...
            ai_summary=DocumentSummary(
                ...
                location_references=[],
                content_type="unknown",
                historical_period=cached_data["ai_summary"].get("historical_period", ""),
                classification_tags=[],
                claude_metadata={},
                processing_time=0.0
            ),
    ...
    def _create_error_result(self, file_path: Path, error_message: str) -> PipelineResult:
    ...
    def generate_pipeline_report(
        ...
        # Calculate averages
        if results:
            report["statistics"]["average_processing_time"] = \
                report["statistics"]["total_processing_time"] / len(results)
        
        if ocr_times:
            report["performance_metrics"]["ocr_average_time"] = \
                sum(ocr_times) / len(ocr_times)
        
        if ai_times:
            report["performance_metrics"]["ai_average_time"] = \
                sum(ai_times) / len(ai_times)
        
        if results:
            report["performance_metrics"]["cache_hit_rate"] = \
                report["statistics"]["cache_hits"] / len(results)
    ...
    def export_results(
        ...
        if format == "json":
            ...
            with open(output_file, 'w', newline='') as f:
                ...
                # Header
                writer.writerow([
                    "File Path", "OCR Confidence", "AI Category",
                    "AI Confidence", "Historical Period", "Suggested Folder",
                    "Processing Time", "Success", "Error"
                ])
            ...
            with open(output_file, 'w') as f:
                ...
                for result in results:
                    ...
                    f.write(f"Suggested Path: {result.ai_summary.suggested_folder_path}\n")
                    f.write("-" * 40 + "\n\n")
