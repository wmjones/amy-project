...
class ResourceMonitor:
    ...
    def get_peak_metrics(self) -> Dict[str, float]:
        ...
        return {
            ...
            'average_memory': np.mean(self.metrics['memory']) if self.metrics['memory'] else 0
        }
    ...
    def create_batches(
        ...
        for file_path in sorted_files:
            ...
            if (len(current_batch) >= self.batch_size or 
                current_size + file_size_mb > self.max_batch_memory):
                if current_batch:
...
class OptimizedPipeline:
    ...
    def _process_batch(
        ...
        logger.info(f"Processing batch {batch_idx + 1}/{total_batches} ({len(batch)} files)")
    ...
    def _calculate_metrics(
        ...
        avg_ocr_time = np.mean(self.metrics['ocr_times']) if self.metrics['ocr_times'] else 0
        avg_ai_time = np.mean(self.metrics['ai_times']) if self.metrics['ai_times'] else 0
    ...
    def analyze_bottlenecks(
        ...
        ocr_percentage = (metrics.ocr_time / total_processing * 100) if total_processing > 0 else 0
        ai_percentage = (metrics.ai_time / total_processing * 100) if total_processing > 0 else 0
        
        if ocr_percentage > 70:
            analysis["bottlenecks"].append("OCR processing")
            analysis["recommendations"].append("Consider using more OCR workers or GPU acceleration")
        
        if ai_percentage > 70:
            analysis["bottlenecks"].append("AI summarization")
            analysis["recommendations"].append("Increase API rate limits or use batch API calls")
        
        # Analyze resource usage
        if metrics.peak_memory_usage > 80:
            analysis["bottlenecks"].append("Memory usage")
            analysis["recommendations"].append("Reduce batch sizes or optimize memory usage")
        
        if metrics.cpu_utilization < 50:
            analysis["recommendations"].append("Increase parallelization to better utilize CPU")
        ...
        if metrics.cache_hit_rate < 0.2:
            analysis["recommendations"].append("Enable or optimize caching for better performance")
    ...
    def generate_performance_report(
        ...
        report = {
            ...
            "timing_breakdown": {
                ...
                "ocr_percentage": (metrics.ocr_time / metrics.average_time_per_file * 100) if metrics.average_time_per_file > 0 else 0,
                "ai_percentage": (metrics.ai_time / metrics.average_time_per_file * 100) if metrics.average_time_per_file > 0 else 0
            },
            "resource_usage": {
            ...
            "optimization_recommendations": self._generate_recommendations(metrics, bottleneck_analysis),
    ...
    def _generate_recommendations(
        ...
        if metrics.throughput < 1.0:  # Less than 60 files per minute
            recommendations.append("For 400+ files, current throughput will take over 7 hours")
            recommendations.append("Consider processing in multiple sessions or overnight")
        
        if metrics.ocr_time > metrics.ai_time * 2:
            recommendations.append("OCR is the primary bottleneck - consider GPU-accelerated OCR")
        ...
        if metrics.peak_memory_usage > 70:
            recommendations.append(f"High memory usage ({metrics.peak_memory_usage:.1f}%) - monitor for OOM errors")
        
        # Scale recommendations for 400+ files
        estimated_time_hours = 400 / (metrics.throughput * 3600) if metrics.throughput > 0 else float('inf')
        if estimated_time_hours > 4:
            recommendations.append(f"Estimated time for 400 files: {estimated_time_hours:.1f} hours")
            recommendations.append("Consider cloud-based processing for better scalability")
