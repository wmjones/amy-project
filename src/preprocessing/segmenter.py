...
class DocumentSegmenter:
    ...
    def _detect_lines(
        self, edges: np.ndarray, horizontal: bool = True
    ...
    def _classify_regions(
        self, regions: List[DocumentRegion], image: np.ndarray
    ) -> List[DocumentRegion]:
    ...
    def visualize_segmentation(
        self, image: np.ndarray, regions: List[DocumentRegion]
    ) -> np.ndarray:
        ...
        for region in regions:
            ...
            cv2.putText(
                vis_image, label, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1
            )
