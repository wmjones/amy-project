...
class PreprocessingPipeline:
    ...
    def __init__(self, config: Optional[PipelineConfig] = None):
        ...
        self.segmenter = DocumentSegmenter(
            ...
            line_detection_threshold=self.config.segmentation[
    ...
    def process_document(self, image_path: str, output_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            (
                metadata.image_characteristics.height,
                metadata.image_characteristics.width,
            ) = original_image.shape[:2]
            metadata.image_characteristics.channels = (
                original_image.shape[2] if len(original_image.shape) > 2 else 1
            )
            ...
            if self.config.output["save_enhanced_images"]:
                ...
                saved_paths = self.enhancer.save_enhanced_images(
                    enhanced_images, output_dir, base_filename
                )
            ...
            metadata.document_regions = [
                self._region_to_dict(region) for region in regions
            ]
            ...
            if self.config.output["create_visualization"]:
                ...
                visualization = self.segmenter.visualize_segmentation(
                    enhanced_images["original"], regions
                )
    ...
    def process_batch(
        ...
        if self.config.pipeline["parallel_processing"]:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                future_to_path = {
                    executor.submit(
                        self.process_document, path, Path(output_dir) / Path(path).stem
                    ): path
            ...
            for i, path in enumerate(image_paths):
                try:
                    result = self.process_document(
                        path, Path(output_dir) / Path(path).stem
                    )
                    results[path] = result
                except Exception as e:
                    logger.error(f"Failed to process {path}: {e}")
