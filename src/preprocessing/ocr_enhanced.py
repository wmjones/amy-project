...
class EnhancedOCRResult:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'EnhancedOCRResult':
        ...
        word_confidences = [WordConfidence(**w) for w in data.get('word_confidences', [])]
        character_confidences = [CharacterConfidence(**c) for c in data.get('character_confidences', [])]
        ...
        problematic_regions = [ProblematicRegion(**r) for r in data.get('problematic_regions', [])]
...
class EnhancedOCRProcessor:
    ...
    def process_variants(
        self,
        image_variants: Dict[str, np.ndarray],
        original_path: str
    ) -> EnhancedOCRResult:
    ...
    def _combine_variant_results(
        ...
        for variant_name, result in variant_results.items():
            ...
            if result['word_confidences']:
                avg_confidence = np.mean([w.confidence for w in result['word_confidences']])
        ...
        for char in all_char_confidences:
            ...
            if key not in unique_chars or char.confidence > unique_chars[key].confidence:
        ...
        for word in result.word_confidences:
            if unusual_pattern.search(word.word):
                region = ProblematicRegion(
                    ...
                    details={'word': word.word, 'characters': unusual_pattern.findall(word.word)},
    ...
    def export_structured_output(
        ...
        if format == 'json':
            ...
            with open(output_path, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(['Type', 'Text', 'Confidence', 'X', 'Y', 'Width', 'Height'])
