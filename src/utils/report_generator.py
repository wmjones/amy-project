...
class ReportGenerator:
    ...
    def record_file_movement(self, source: Path, destination: Path, 
                           metadata: Dict[str, Any], rule_name: str):
    ...
    def record_error(self, file_path: Path, error: Exception, 
    ...
    def generate_summary_report(self, output_format: str = 'text',
                              total_files: int = 0,
                              processed_files: int = 0,
                              successful_files: int = 0,
                              failed_files: int = 0,
                              skipped_files: int = 0) -> str:
        """
    ...
    def _generate_text_report(self, stats: Dict[str, Any]) -> str:
        ...
        if summary['total_files'] > 0:
            ...
            processed_pct = 0
        report.append(f"Processed:        {summary['processed_files']:,} ({processed_pct:.1f}%)")
        report.append(f"Successful:       {summary['successful_files']:,} ({summary['success_rate']:.1f}%)")
        ...
        if stats['rule_usage']:
            ...
            report.append("-" * 10)
            for rule, count in sorted(stats['rule_usage'].items(), 
        ...
        if stats['type_distribution']:
            ...
            report.append("-" * 21)
            for file_type, count in sorted(stats['type_distribution'].items(), 
                                         key=lambda x: x[1], reverse=True):
        ...
        if stats['error_count'] > 0:
            ...
            if stats['error_count'] > len(stats['errors']):
                report.append(f"... and {stats['error_count'] - len(stats['errors'])} more errors")
            report.append("")
    ...
    def _generate_html_report(self, stats: Dict[str, Any]) -> str:
        ...
        if stats['rule_usage']:
            ...
            for rule, count in sorted(stats['rule_usage'].items(), 
                                    key=lambda x: x[1], reverse=True):
                html_parts.append(f'<tr><td>{html.escape(rule)}</td><td>{count:,}</td></tr>')
            html_parts.append('</table>')
        ...
        for file_type, count in sorted(stats['type_distribution'].items(), 
                                     key=lambda x: x[1], reverse=True)[:10]:
            html_parts.append(f'<tr><td>{html.escape(file_type)}</td><td>{count:,}</td></tr>')
        ...
        for size_range in size_order:
            if size_range in stats['size_distribution']:
                count = stats['size_distribution'][size_range]
                html_parts.append(f'<tr><td>{html.escape(size_range)}</td><td>{count:,}</td></tr>')
