    ...
    def _build_tree(
        ...
        try:
            ...
            for i, entry in enumerate(entries):
                ...
                # Add entry to tree
                lines.append(
                    f"{prefix}{connector}{self.tree_chars['horizontal']*2} {entry.name}"
                )
            ...
            lines.append(
                f"{prefix}{self.tree_chars['branch']}{self.tree_chars['horizontal']*2} [Permission Denied]"
            )

    def compare_structures(
        self, before_path: Path, after_path: Path, max_depth: int = 3
    ) -> Dict[str, str]:
        """
        ...
        """
        before_tree = self.visualize_directory_tree(
            before_path, max_depth, show_files=True
        )
        after_tree = self.visualize_directory_tree(
            after_path, max_depth, show_files=True
        )
