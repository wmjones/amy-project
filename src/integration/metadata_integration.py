...
class MetadataIntegrationBridge:
    ...
    def __init__(
        ...
        logger.info(
            f"Metadata integration bridge initialized with {conflict_resolution_strategy} strategy"
        )
        ...
        try:
            ...
            if conflicts and not force_update:
                resolved_metadata = self._resolve_conflicts(
                    merged_metadata, conflicts, existing_metadata, ai_metadata
                )
            else:
    ...
    def _convert_ai_to_document_metadata(
        ...
        if ai_summary.date_references:
            date_info.document_date = self._normalize_date(
                ai_summary.date_references[0]
            )
            date_info.mentioned_dates = [
                self._normalize_date(d)
                for d in ai_summary.date_references[1:]
                if self._normalize_date(d)
            ]

        # Create DocumentMetadata
        return DocumentMetadata(
            document_type=self._map_content_type_to_document_type(
    ...
    def _merge_metadata(
        ...
        # Handle summary
        if ai_generated.summary and (
            not existing.summary or len(ai_generated.summary) > len(existing.summary)
        ):
        ...
        if ai_generated.extracted_text and len(ai_generated.extracted_text) > len(
            existing.extracted_text or ""
        ):
    ...
    def update_organization_path(
        self, metadata: DocumentMetadata, use_ai_suggestion: bool = True
    ) -> Tuple[str, str]:
        ...
        target_path, rule_name = self.organization_engine.determine_target_location(
            metadata
        )
        ...
        if (
            use_ai_suggestion
            and metadata.suggested_folder
            and metadata.confidence_score >= 0.85
        ):
    ...
    def create_enriched_metadata(
        self, pipeline_results: List[PipelineResult], batch_name: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        for result in pipeline_results:
            if result.success:
                ...
                entity_count = sum(
                    len(v) for v in result.ai_summary.key_entities.values()
                )
                report["enrichment_statistics"][
                ...
                if period:
                    report["syracuse_specific"]["historical_periods"][period] = (
                        report["syracuse_specific"]["historical_periods"].get(period, 0)
                        + 1
                    )
    ...
    def validate_integration(
        ...
        if (
            integrated_metadata.confidence_score < 0
            or integrated_metadata.confidence_score > 1
        ):
        ...
        if integrated_metadata.suggested_folder:
            invalid_chars = '<>:"|?*'
            if any(
                char in integrated_metadata.suggested_folder for char in invalid_chars
            ):
                validation["warnings"].append(
                    "Suggested folder contains invalid characters"
                )
        ...
        if original_metadata:
            ...
            if (
                integrated_metadata.dates.document_date
                and not original_metadata.dates.document_date
            ):
                validation["improvements"].append("Added document date")

            if (
                integrated_metadata.confidence_score
    ...
    def _log_audit_event(
        self, event_type: str, file_path: str, details: Dict[str, Any]
    ):
    ...
    def get_audit_trail(
        ...
        if event_type:
            filtered_trail = [
                e for e in filtered_trail if e["event_type"] == event_type
            ]

        if start_date:
            filtered_trail = [
                e
                for e in filtered_trail
