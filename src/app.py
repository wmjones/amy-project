...
class FileOrganizerApp:
    ...
    def initialize(self):
        ...
        try:
            ...
            self.config_manager = ConfigManager(config_file=Path(self.config_file) if self.config_file else None)
    ...
    def run(self, source_override: Optional[str] = None, 
            target_override: Optional[str] = None,
            dry_run_override: Optional[bool] = None) -> bool:
        ...
        if target_override:
            ...
            self.components['file_manipulator'] = FileManipulator(
                ...
                dry_run=dry_run_override if dry_run_override is not None else self.config.get('dry_run', False)
            )
        ...
        try:
            ...
            logger.info(f"Successfully processed: {self.progress_tracker.success_count}")
    ...
    def _organize_files(self, processed_files: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for processed in processed_files:
            try:
                ...
                target_path, rule_name = organization_engine.determine_target_location(metadata)
                ...
                if isinstance(file_accessor, DropboxAccessor):
                    ...
                    if mode == 'move':
                        success = file_accessor.move_file(file.path, target_dropbox_path)
                    else:  # copy
                        success = file_accessor.copy_file(file.path, target_dropbox_path)
                else:
                    # For local files, use file manipulator
                    source_path = file.path if hasattr(file, 'path') else processed['file_path']
                    
                    # Use organize_file method which handles both move and copy
                    success = file_manipulator.organize_file(source_path, target_path, mode)
                
                if success:
                    self.progress_tracker.update_progress(
                        str(file),
                        'success',
                        f"Organized to: {target_path}"
                    )
                    organized_results.append({
                        'file': file,
                        'source': file.path if hasattr(file, 'path') else file.name,
    ...
    def _generate_report(self, organized_results: List[Dict[str, Any]]) -> str:
        ...
        report_generator = ReportGenerator(
            ...
            self.components['file_manipulator']
        )
        ...
        # Save report to file
        report_dir = Path(self.config.get('organization', {}).get('base_directory', './organized')) / 'reports'
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        '--source',
        help='Source directory or Dropbox path',
        default=None
    )
