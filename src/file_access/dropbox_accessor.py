...
class DropboxAccessor:
    ...
    def _authenticate(self):
        ...
        try:
            if self.refresh_token:
                ...
                logger.info(
                    "Successfully authenticated with Dropbox using refresh token"
                )
                ...
                self.client.users_get_current_account()
                logger.info(
                    "Successfully authenticated with Dropbox using access token"
                )
    ...
    def download_file(
        self, dropbox_path: str, local_path: Path, show_progress: bool = True
    ) -> Path:
        """
        ...
        try:
            ...
            with open(local_path, "wb") as f:
                if show_progress and file_size > 0:
                    with tqdm(
                        total=file_size,
                        desc=f"Downloading {Path(dropbox_path).name}",
                        unit="B",
                        unit_scale=True,
                    ) as pbar:
    ...
    def download_batch(
        self, file_paths: List[str], local_dir: Path, max_concurrent: int = 5
    ) -> Dict[str, Path]:
        """
    ...
    def move_file(self, from_path: str, to_path: str, autorename: bool = True) -> bool:
        """
        ...
        try:
            self.client.files_move_v2(from_path, to_path, autorename=autorename)
    ...
    def copy_file(self, from_path: str, to_path: str, autorename: bool = True) -> bool:
        """
        ...
        try:
            self.client.files_copy_v2(from_path, to_path, autorename=autorename)
    ...
    def upload_file(
        self, local_path: Path, dropbox_path: str, overwrite: bool = False
    ) -> FileMetadata:
        """
            ...
            with open(local_path, "rb") as f:
                ...
                if file_size > 150 * 1024 * 1024:  # 150MB
                    ...
                    metadata = self.client.files_upload(
                        f.read(), dropbox_path, mode=mode, mute=True
                    )
    ...
    def _chunked_upload(self, file_obj, dropbox_path: str, file_size: int):
        ...
        with tqdm(
            total=file_size,
            desc=f"Uploading {Path(dropbox_path).name}",
            unit="B",
            unit_scale=True,
        ) as pbar:
    ...
    def search_files(self, query: str, max_results: int = 100) -> List[DropboxFile]:
        """
