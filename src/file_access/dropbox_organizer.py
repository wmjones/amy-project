...
class DropboxOrganizer:
    ...
    def __init__(self, config: ConfigManager,
        ...
        # Configuration
        self.source_folder = config.get('dropbox.source_folder', '/file-organizer-uploads')
    ...
    def organize_dropbox_files(self, 
    ...
    def _process_batch(self, batch: List[DropboxFile],
                      progress_tracker: Optional[ProgressTracker]) -> Dict[str, Any]:
        ...
        for dropbox_file, local_path in download_results.items():
            if local_path is None:
                results['failed'] += 1
                if progress_tracker:
                    progress_tracker.update_file(dropbox_file.path, "failed", 
            ...
            try:
                ...
                if progress_tracker:
                    progress_tracker.update_file(dropbox_file.path, "failed", error=str(e))
    ...
    def _download_batch(self, batch: List[DropboxFile]) -> Dict[DropboxFile, Optional[Path]]:
    ...
    def _process_single_file(self, dropbox_file: DropboxFile,
        ...
        try:
            ...
            metadata = self.metadata_extractor.extract_from_processed_file(processed_file)
            ...
            if success:
                ...
                if self.cleanup_after_org and self.config.get('organization.mode') == 'copy':
                    try:
    ...
    def scan_and_preview(self, source_folder: Optional[str] = None,
        ...
        for file in dropbox_files[:5]:
            try:
                ...
                if processed:
                    metadata = self.metadata_extractor.extract_from_processed_file(processed)
                    rule = self.organization_engine.find_matching_rule(metadata)
