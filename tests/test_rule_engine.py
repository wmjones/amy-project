...
class TestRuleEngine:
    ...
    def test_evaluate_nested_conditions(self):
        ...
        # Nested field match
        rule = {
            'conditions': {'dates.document_date': '2023-10-15'},
            'enabled': True
        }
        assert self.engine.evaluate_rule(rule, self.metadata) is True
        ...
        # Array access
        rule = {
            'conditions': {'entities.people[0]': 'John Doe'},
            'enabled': True
        }
        assert self.engine.evaluate_rule(rule, self.metadata) is True
        ...
        rule = {
            'conditions': {'entities.people[5]': 'Someone'},
            'enabled': True
        }
        assert self.engine.evaluate_rule(rule, self.metadata) is False
    ...
    def test_evaluate_operators(self):
        ...
        # Exists operator
        rule = {
            'conditions': {'amounts.total': {'$exists': True}},
            'enabled': True
        }
        assert self.engine.evaluate_rule(rule, self.metadata) is True
    ...
    def test_string_operators(self):
        ...
        # Ends with
        rule = {
            'conditions': {'filename': {'$ends_with': '.pdf'}},
            'enabled': True
        }
        assert self.engine.evaluate_rule(rule, metadata) is True
    ...
    def test_apply_path_template(self):
        ...
        template = 'Invoices/{entities.organizations[0]}/{dates.document_date|%Y}/{filename}'
        path = self.engine.apply_path_template(template, self.metadata, 'invoice_001.pdf')
    ...
    def test_operator_type_safety(self):
        ...
        rule = {
            'conditions': {'document_type': {'$gt': 100}},
            'enabled': True
        }
        assert self.engine.evaluate_rule(rule, self.metadata) is False
        ...
        # Regex on non-string
        rule = {
            'conditions': {'amounts.total': {'$regex': r'\d+'}},
            'enabled': True
        }
        assert self.engine.evaluate_rule(rule, self.metadata) is False
        ...
        rule = {
            'conditions': {'amounts.total': {'$contains': 100}},
            'enabled': True
        }
        assert self.engine.evaluate_rule(rule, self.metadata) is False
