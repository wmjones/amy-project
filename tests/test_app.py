...
class TestFileOrganizerApp(unittest.TestCase):
    ...
    def test_component_initialization(self, *mocks):
        ...
        # Unpack mocks
        (mock_batch_processor, mock_error_handler, mock_file_manipulator,
         mock_org_engine, mock_metadata_extractor, mock_claude_client,
         mock_file_processor, mock_file_accessor, mock_config_manager) = mocks
    ...
    def test_source_and_target_override(self):
        ...
        with patch.object(app, '_scan_files', return_value=[]), \
             patch.object(app, '_cleanup'), \
             patch('src.app.FileSystemAccessor') as mock_accessor, \
             patch('src.app.FileManipulator') as mock_manipulator:
    ...
    def test_file_processing_workflow(self):
        ...
        mock_org_engine.determine_target_location.return_value = ('target/path', 'test_rule')
