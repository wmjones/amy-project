...
class TestFolderVisualizer:
    ...
    def test_visualize_with_file_filter(self):
        ...
        tree = self.visualizer.visualize_directory_tree(
            self.temp_dir, max_depth=3, show_files=True, file_extensions={".pdf"}
        )
    ...
    def test_compare_structures(self):
        ...
        assert (
            "Added files:" in comparison["diff"]
            or "Removed files:" in comparison["diff"]
        )
    ...
    def test_export_structure_as_json(self):
        ...
        structure = self.visualizer.export_structure_as_json(self.temp_dir, max_depth=2)
        ...
        # Find docs directory
        docs_dir = next(
            child for child in structure["children"] if child["name"] == "docs"
        )
        ...
        readme = next(
            (child for child in structure["children"] if child["name"] == "readme.txt"),
            None,
        )
        if readme:
