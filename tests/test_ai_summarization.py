...
class TestAISummarizer(unittest.TestCase):
    ...
    def test_summarize_document_success(self):
        ...
        summary = self.summarizer.summarize_document(
            file_path=self.test_file, ocr_text=self.test_text, ocr_confidence=0.9
        )
        ...
        self.assertIn(
            "Syracuse Salt Company", summary.key_entities.get("organizations", [])
        )
    ...
    def test_summarize_document_error(self):
        ...
        # Test summarization
        summary = self.summarizer.summarize_document(
            file_path=self.test_file, ocr_text=self.test_text
        )
    ...
    def test_batch_summarize(self):
        ...
        summaries = self.summarizer.batch_summarize(
            documents, batch_size=2, progress_callback=progress_callback
        )
        ...
        )  # Called twice for 3 docs with batch_size=2
...
class TestOCRAIPipeline(unittest.TestCase):
    ...
    def test_process_batch(self):
        ...
        results = self.pipeline.process_batch(
            files, max_workers=2, progress_callback=progress_callback
        )
    ...
    def test_caching(self):
        ...
        # First processing
        with patch.object(
            self.pipeline, "_calculate_file_hash", return_value="test_hash"
        ):
